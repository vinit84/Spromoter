...
{
    public function index(ModerationDataTable $dataTable)
    {
        $store = activeStore();
        ...
        return $dataTable
            ...
            ->render('user.reviews.moderation.index', [
            'store' => $store,
            'products' => $products,
    ...
    public function comment(StoreCommentRequest $request)
    {
        ...
        return success(trans('Comment has been added.'), data: [
            'comment' => view('user.reviews.moderation.table.comment', compact('comment'))->render(),
        ]);
    }
