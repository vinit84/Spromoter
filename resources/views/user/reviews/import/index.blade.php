...
{{--                                <option value="feefo" data-tab="#navs-feefo">{{ trans('Feefo') }}</option>--}}
                                ...
                                <option value="spromoter" data-tab="#navs-spromoter">{{ trans('SPromoter') }}</option>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="tab-content">
                        ...
                        <div class="tab-pane fade" id="navs-spromoter">
                            <h5 class="card-title">
                                ...
                                <ol>
                                    <li>
                                        {!! trans('Download the CSV file template. :link', [
                                            'link' => '<a href="'.asset('assets/files/SPromoter Review Template.xlsx').'" target="_blank">'.trans('Download').'<i class="ti ti-download"></i></a>'
                                        ]) !!}
                                    </li>
                                    <li>
                        ...
                        <div class="tab-pane fade" id="navs-other">
                            <h5 class="card-title">
                                {{ trans('Importing reviews using :name template', ['name' => trans('other')]) }}
                            </h5>
                            <div class="card-text">
                                <ol>
                                    <li>
                                        {!! trans('Download the CSV file template. :link', [
                                            'link' => '<a href="'.asset('assets/files/Other Import Template.xlsx').'" target="_blank">'.trans('Download').'<i class="ti ti-download"></i></a>'
                                        ]) !!}
                                    </li>
            ...
            <div class="modal-content p-3 p-md-5">
                <div class="modal-body">
                    ...
                    <div class="text-center mb-4">
                        <h3 class="mb-2">{{ trans('Are you sure!') }}</h3>
                        <p class="text-muted">
                            {{ trans('By clicking "Yes, Import" you will import the following file.') }}
                        </p>
                    </div>
                    <div class="row g-3">
                        <div class="col-12">
...
@push('pageScripts')
    <script>
        let store = {{ Js::from(activeStore()->uuid) }}
    </script>
    <script>
        $(function () {
            ...
            const pond = FilePond.create(document.getElementById('file'), {
                ...
                server: {
                    process: {
                        url: route('user.reviews.import.store', store),
                        headers: {
                    ...
                    revert: {
                        url: route('user.reviews.import.delete-temporary-file', store),
                        headers: {
                            'X-CSRF-TOKEN': CSRF_TOKEN,
                        }
                    }
            ...
            pond.on('processfile', (error, file) => {
                ...
                if (response.status === 'success') {
                    ...
                    $('#confirmImportButton').attr('href', route('user.reviews.import.confirm', {store, file: response.data.folder, provider: response.data.provider}))
